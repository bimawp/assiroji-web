...
import { bucket, prisma, supabase, supabaseAnonKey, supabaseUrl, verifyToken } from '@/lib/prisma';
...
export async function DELETE(req, context) {
  try {
    ...
    const { data: deleteOldData, error: errorDeleteOldData } = await supabaseAuth.storage
      .from(bucket)
      .remove([oldDataImage]);
    await deleteRecord('Ekstrakurikuler', { id_ekstrakurikuler: id });
    if (errorDeleteOldData) {
      throw new Error(`Error uploading image: ${errorDeleteOldData.message}`);
    }
